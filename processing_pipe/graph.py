...
class Graph(object):
	...
	def run(self):
		...
		while True:
			...
			computation_result = dict(
			    outputs=[
			        self.cellList[self._outputs[i][0]].outputs[self._outputs[i][1]]\
	...
	def setPortAsGraphOutput(self, cell_id, port_name, *args, **kwargs):
		...
		"""
		self._outputs.append((cell_id, str(port_name)))
	...
	def setPortAsGraphInput(self, cell_id, port_name, *args, **kwargs):
		...
		:param kwargs: Other arguments (ignored)
		"""
		self._inputs_handler.addInput(cell_id, str(port_name))
	...
	def setSwitchingParameters(self, cell_id, param_name, values):
		...
		"""
		self._params_handler.setParameterPossibleValues(cell_id,
		                                                param_name,
		                                                values)
	...
	def input(self, value):
		"""
		An alternative way to set graph input values when there is only one
		input port.
		"""
